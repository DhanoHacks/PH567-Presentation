...
def plot_bifurcation_diagram(A, points, name, save=False):
    ...
    plt.xlabel("A")
    plt.ylabel("Asymptotic Trajectory")
...
def main(mode):
    ...
    full = False
    hd = False
    (chosen_map,left,right,name) = [(logistic_map,0 if full else 2.9,4,"Logistic Map"),(sine_map,0 if full else 2,np.pi,"Sine Map"),(quadratic_map,0,2,"Quadratic Map"),(biquadratic_map,0,2,"Biquadratic Map")][mode]
    A = np.arange(left,right,0.001 if hd else 0.003)
